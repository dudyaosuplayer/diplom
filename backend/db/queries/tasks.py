...
def get_task_by_task_id(task_id: int, db: Session) -> Task:
...
def get_task_by_from_project(project_id: int, task_id: int, db: Session) -> Task:
    return db.query(Task).filter(Task.project_id == project_id, Task.id == task_id).first()


def get_tasks_by_user_in_project(project_id: int, user_id: int, db: Session) -> List[Task]:
    return db.query(Task).filter(Task.project_id == project_id,
